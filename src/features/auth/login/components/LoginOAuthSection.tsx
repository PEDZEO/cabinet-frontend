...
export function LoginOAuthSection({
  ...
  return (
    <div className={compact ? 'auth-oauth-section auth-oauth-section--compact' : undefined}>
      {showDivider && (
        <div className="auth-oauth-divider my-4 flex items-center gap-3">
      ...
      <div className="auth-oauth-list flex items-stretch gap-2">
        {isLoading
          ? [0, 1, 2].map((idx) => (
              <div
                key={`oauth-skeleton-${idx}`}
                className="auth-oauth-button flex flex-1 flex-col items-center justify-center gap-1.5 rounded-xl border border-dark-700 bg-dark-800/60 py-2.5"
              >
          ...
          : providers.map((provider) => (
              <button
                ...
                className="auth-oauth-button flex flex-1 flex-col items-center justify-center gap-1.5 rounded-xl border border-dark-700 bg-dark-800/80 py-2.5 transition-all hover:border-dark-600 hover:bg-dark-700 disabled:opacity-50"
